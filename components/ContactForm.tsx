...
const inputCls = "w-full rounded-[11px] border-[1.5px] border-line bg-[#fffdfb] px-3.5 py-3 text-[14.5px] text-ink outline-none transition focus:border-wine";
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (<div className="mb-4"><label className="mb-1.5 block text-[13px] font-semibold text-wine">{label}</label>{children}</div>);
}
    ...
    <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="rounded-xl2 border border-line bg-white p-7 shadow-soft">
      {sent ? (
        <div className="py-10 text-center">
          <div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-full bg-wine-soft text-2xl">✓</div>
          <h3 className="font-display text-xl font-semibold text-wine">¡Gracias!</h3>
        ...
        <>
          ...
          <Field label="Nivel de interés">
            <select className={inputCls}><option>Preescolar</option><option>Primaria</option><option>Secundaria</option><option>Preparatoria</option></select>
          </Field>
