...
export default function LazyMap({ className = "" }: { className?: string }) {
  ...
  return (
    <div ref={ref} className={`relative h-[320px] w-full overflow-hidden rounded-xl2 border border-line bg-bg-2 ${className}`}>
      {show ? (
        <iframe title="Ubicación del Colegio Pedro de Gante" src={MAPS_EMBED} loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="h-full w-full border-0" />
      ) : (
        <a href={MAPS_LINK} target="_blank" rel="noopener noreferrer" className="flex h-full w-full flex-col items-center justify-center gap-2 text-muted">
          ...
          <span className="font-display text-sm font-semibold text-wine">Ver ubicación en Google Maps</span>
          <span className="text-[13px]">Av. Vicente Riva Palacio #200 · Tulancingo</span>
        </a>
      )}
    </div>
  );
