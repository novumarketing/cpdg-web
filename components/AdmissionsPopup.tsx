...
export default function AdmissionsPopup() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("pdg_adm_popup") === "1") return;
  ...
  const close = () => { setOpen(false); try { localStorage.setItem("pdg_adm_popup", "1"); } catch {} };
  if (!open) return null;
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="adm-title" className="fixed inset-0 z-[60] grid place-items-center p-4">
      <div className="absolute inset-0 bg-wine-2/60 backdrop-blur-sm" onClick={close} />
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-soft">
        <div className="bg-gradient-to-br from-wine to-wine-2 px-7 py-8 text-center text-white">
          ...
          <div className="font-sans text-[12px] font-semibold uppercase tracking-[0.2em] text-gold">Admisiones 2026–2027</div>
          <h2 id="adm-title" className="mt-2 font-display text-2xl font-bold">Pase directo a las mejores universidades</h2>
        </div>
        <div className="px-7 py-6 text-center">
          <p className="text-[15px] text-muted">Asegura el lugar de tu hijo en el Colegio Pedro de Gante. Cupo limitado por grupo.</p>
